          ...
          <section className="relative min-h-[85vh] lg:min-h-[calc(100vh-90px)] flex flex-col items-center justify-center overflow-hidden bg-[#030614] py-16 px-4 sm:px-6 lg:px-8 text-center shadow-2xl">
            ...
            <div className="relative z-10 max-w-4xl mx-auto space-y-7 flex flex-col items-center mt-[-40px]">
              <div className="inline-flex items-center gap-2 rounded-full border border-rose-500/40 bg-rose-950/60 px-5 py-2 text-xs sm:text-sm font-extrabold uppercase tracking-widest text-rose-200 backdrop-blur-md shadow-lg shadow-rose-900/30 hover:border-rose-400/60 transition-colors">
                <span className="text-amber-400 animate-pulse">✨</span> {cms?.hero?.badgeText || "100% VERIFIED INDEPENDENT MODELS & ESCORT DIRECTORY"}
              </div>

              <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black tracking-tight text-white leading-[1.1] drop-shadow-2xl">
                {cms?.hero?.titlePrefix || "Connect with Genuine"}{" "}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-pink-500 to-amber-300">
                  {cms?.hero?.titleHighlight || "Independent Escorts"}
                </span>
              </h1>

              <p className="text-sm sm:text-lg text-slate-300 leading-relaxed max-w-2xl font-medium drop-shadow-lg">
              ...
              <div
                ...
                className="w-full max-w-2xl mt-6 cursor-pointer group rounded-full border border-rose-500/40 bg-slate-900/80 hover:bg-slate-900 p-2 sm:p-2.5 shadow-[0_0_40px_-10px_rgba(225,29,72,0.3)] hover:shadow-[0_0_50px_-5px_rgba(225,29,72,0.4)] backdrop-blur-xl transition-all duration-300 flex items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3 px-4 flex-1 overflow-hidden">
                  <Search className="h-5 w-5 text-rose-400 shrink-0 group-hover:scale-110 group-hover:text-rose-300 transition-all" />
                  <span className="text-[13px] sm:text-[15px] text-slate-300 font-semibold truncate text-left">
                    Search by Nationality, Category, City, Service or Filters...
                  </span>
                </div>
                <button
                  type="button"
                  className="rounded-full bg-gradient-to-r from-rose-600 via-pink-600 to-purple-600 px-6 sm:px-8 py-3.5 text-xs sm:text-[14px] font-black uppercase tracking-wider text-white shadow-lg group-hover:scale-105 transition-transform shrink-0 flex items-center gap-2"
                >
